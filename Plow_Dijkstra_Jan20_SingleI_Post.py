...
def getPose():
    ...
    # Assumes marker onde is on the back of the plow, with three facing forward. If it was a compass, NWSE would be 3412.
    # while arduino.inWaiting():
    while True:
...
def moveToWaypoint(wp):
    ...
    while True:
        ...
        if not stopSignDetected:
            print("Stopsign detected, stopping vehicle")
...
for waypoint in waypoints:
    ...
    while not atWaypoint:
        ssDetected, scene = detectStopsign()
        postDetected = detectPost2M.detectPost(scene)
        if postDetected:
            print("Found a post!")
        #getPose()
